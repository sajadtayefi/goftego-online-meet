            ...
            {isOpen ?
                ...
                <div className={`${isOpen ? 'translate-x-0' : '"-translate-x-full"'}transition-transform px-5  duration-300 ease-linear xl:hidden flex w-1/2 h-full fixed  top-0 z-20 right-0  flex-col justify-start  pt-5 items-start  bg-white `}>
                    ...
                    {HeaderItems.map((header, index) => (
                        <p key={index} className={`${header.isActive ? " text-SecendryDark" : "text-black"} flex w-full h-10 xl:h-20 justify-start border-b cursor-pointer   items-center mx-auto text-[12px] lg:text-base font-medium list-none `} >
                            {header.name}
                        </p>
                    ))}
                </div>
                ...
                <div className=" py-8 flex flex-row justify-start basis-full mr-10    items-center ">
                    {HeaderItems.map((header, index) => (
                        <Link key={index} rel="stylesheet" href={header?.link}>
